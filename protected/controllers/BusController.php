...
class BusController extends Controller
{
	...
	public function actionCreate()
	{
		...
		$vehicleTypeId                   = VehicleType::model()->findByAttributes(array('vehicle_type' => self::VEHICLE_TYPE_BUS))->vehicle_typeid;
...
//		$this->performAjaxValidation(array($modelVehicle, $modelBus, $modelPhysicalPerson, $modelCompany, $modelVehicleAdvertisement));
		...
		if (isset($_POST['Vehicle']))
		{
			...
			if ($modelVehicle->validate()) {
				if ($modelVehicle->save(false)) {
					...
					$modelBus->vehicleid = $vehicleId;

					if($modelBus->validate()) {
						if ($modelBus->save(false)) {
	...
	public function actionUpdate($id)
	{
		...
		if (isset($_POST['Bus'])) {
			...
			if ($model->save())
				$this->redirect(array('view', 'id' => $model->busid));
		}

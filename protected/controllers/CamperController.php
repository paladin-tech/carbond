...
class CamperController extends Controller
{
	...
	const VEHICLE_TYPE_CAMPER = 'campers';
	...
	public function actionCreate()
	{
		...
		$vehicleTypeId                   = VehicleType::model()->findByAttributes(array('vehicle_type' => self::VEHICLE_TYPE_CAMPER))->vehicle_typeid;
...
//		$this->performAjaxValidation(array($modelVehicle, $modelCamper, $modelPhysicalPerson, $modelCompany, $modelVehicleAdvertisement));
		...
		if (isset($_POST['Vehicle'])) {
			...
			$modelVehicle->vehicle_typeid = $vehicleTypeId;
			$modelCamper->attributes      = $_POST['Camper'];
			...
			if ($modelVehicle->validate()) {
				if ($modelVehicle->save(false)) {
					...
					$modelCamper->vehicleid = $vehicleId;

					if ($modelCamper->validate()) {
						if ($modelCamper->save(false)) {
	...
	public function actionUpdate($id)
	{
		...
		if (isset($_POST['Camper'])) {
			$model->attributes = $_POST['Camper'];
			if ($model->save())
				$this->redirect(array('view', 'id' => $model->camperid));
		}
	...
	{
		$model = new Camper('search');
		$model->unsetAttributes(); // clear any default values
		if (isset($_GET['Camper']))
			$model->attributes = $_GET['Camper'];
	 ...
	 * @return Camper the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model = Camper::model()->findByPk($id);
		if ($model === null)
	 ...
	 * @param Camper $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if (isset($_POST['ajax']) && $_POST['ajax'] === 'camper-form') {

...
class TrailerController extends Controller
{
	...
	const VEHICLE_TYPE_TRAILER = 'trailers';
	...
	public function actionCreate()
	{
		...
		$vehicleTypeId                   = VehicleType::model()->findByAttributes(array('vehicle_type' => self::VEHICLE_TYPE_TRAILER))->vehicle_typeid;
		$modelVehicle                    = new Vehicle;
		$modelVehicle->vehicle_typeid    = $vehicleTypeId;
		$modelVehicleAdvertisement       = new VehicleAdvertisment();
		$modelTrailer                    = new Trailer;
		...
		if (isset($_POST['Vehicle']) && isset($_POST['Trailer'])) {
			// Add Vehicle first
			$modelVehicle->attributes     = $_POST['Vehicle'];
			$modelVehicle->vehicle_typeid = $vehicleTypeId;
			$modelTrailer->attributes     = $_POST['Trailer'];
			$modelParty->save();
			...
			if ($modelVehicle->validate() && $modelTrailer->validate()) {
				if ($modelVehicle->save()) {
					...
					$modelTrailer->vehicleid = $vehicleId;

					if ($modelTrailer->save()) {
	...
	{
		$dataProvider = new CActiveDataProvider('Trailer');
	...
	{
		$model = new Trailer('search');
		$model->unsetAttributes(); // clear any default values
		if (isset($_GET['Trailer']))
			$model->attributes = $_GET['Trailer'];
	...
	{
		$model = Trailer::model()->findByPk($id);
		if ($model === null)
	 ...
	 * @param Trailer $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if (isset($_POST['ajax']) && $_POST['ajax'] === 'trailer-form') {
